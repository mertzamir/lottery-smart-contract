...
def get_account(index=None, id=None):
    # accounts[0] -> ganache dev chain accounts
    # accounts.add("env") -> from .env file
    # accounts.load("id") -> from brownie accounts
    if index:
...
def get_contract(contract_name):
    ...
    if network.show_active() in LOCAL_BLOCKCHAIN_ENVIRONMENTS:
        if len(contract_type) <= 0:  # not deployed and
            deploy_mocks()
        contract = contract_type[-1]
    else:  # not in local dev chain
